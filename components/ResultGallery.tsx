...
export const ResultGallery: React.FC<ResultGalleryProps> = ({ 
  ...
  return (
    <>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                {section.items.map((img) => {
                  ...
                  const containerClass = isReview 
                     ? "bg-white rounded-xl shadow-lg border border-slate-200 hover:shadow-xl transition-all duration-300 overflow-hidden transform hover:-translate-y-1"
                     : "bg-white rounded-3xl shadow-xl hover:shadow-2xl shadow-indigo-100 hover:shadow-violet-200 overflow-hidden transition-all duration-300 flex flex-col h-full transform hover:-translate-y-2 ring-1 ring-black/5";
                      ...
                      <div className="p-4 flex-grow bg-white relative z-10 flex flex-col justify-between">
                        ...
                        {img.textReview && isReview && (
                          <div className="mt-1">
                             ...
                             <p className="text-sm text-slate-700 leading-snug">"{img.textReview}"</p>
      ...
      {editingImage && (
        <div className="fixed inset-0 z-[100] bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4 animate-fadeIn">
          <div className="bg-white rounded-3xl p-8 max-w-lg w-full shadow-2xl transform transition-all scale-100 ring-1 ring-white/20">
             ...
             <div className="mb-6">
               <label className="block text-sm font-bold text-slate-700 mb-2">{t.singleRegen.placeholder}</label>
               <textarea
             ...
             <div className="flex justify-end space-x-3">
               <button onClick={() => setEditingImage(null)} className="px-6 py-3 rounded-full font-bold text-slate-500 hover:bg-slate-100 transition-colors">{t.singleRegen.cancel}</button>
               <button onClick={submitRegeneration} disabled={!feedback.trim()} className={`px-8 py-3 rounded-full font-bold text-white shadow-lg transition-all ${feedback.trim() ? 'bg-violet-600 hover:bg-violet-700 shadow-violet-500/30' : 'bg-slate-300 cursor-not-allowed'}`}>{t.singleRegen.submit}</button>
             </div>
          </div>
        </div>
      )}
      ...
      {/* Full Screen Modal */}
      {selectedImage && (
        <div className="fixed inset-0 z-[100] bg-slate-900/95 flex items-center justify-center p-4 backdrop-blur-md animate-fadeIn" onClick={() => setSelectedImage(null)}>
          <div className="relative max-w-7xl max-h-[95vh] w-full h-full flex items-center justify-center">
             <button onClick={() => setSelectedImage(null)} className="absolute top-4 right-4 z-50 text-white/70 hover:text-white bg-white/10 hover:bg-white/20 rounded-full p-2 transition-colors">
             ...
             <div className="relative w-auto h-auto max-w-full max-h-full flex flex-col items-center">
                <img src={selectedImage.url} alt="Full view" className="max-w-full max-h-[85vh] object-contain rounded-2xl shadow-2xl ring-1 ring-white/10" onClick={(e) => e.stopPropagation()} />
             ...
             <a href={selectedImage.url} download={`product-ai-${selectedImage.id}.png`} onClick={(e) => e.stopPropagation()} className="absolute bottom-8 left-1/2 transform -translate-x-1/2 bg-white text-violet-700 px-10 py-4 rounded-full font-bold shadow-2xl hover:bg-violet-50 hover:scale-105 transition-all flex items-center">
